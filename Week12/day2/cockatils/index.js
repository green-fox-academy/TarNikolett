...
app.get('/', (req, res) => {
    if(req.query.alcohol){
        let containsSpecificAlcohol = cocktails.filter( element => 
            element.contains.includes(req.query.alcohol)
        );
        res.render('home', {
          title: 'Cocktails',
            cocktails : containsSpecificAlcohol,
...
app.listen(port, () => {
    console.log(`Az app a ${port}-es porton fut!`)
});